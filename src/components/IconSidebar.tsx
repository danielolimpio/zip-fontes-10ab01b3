                ...
                >
                  {'customIcon' in item && item.customIcon ? (
                    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none">
                      {/* Paleta base */}
                      <circle cx="12" cy="12" r="10" stroke={isActive(item) ? 'currentColor' : '#9E9E9E'} strokeWidth="2" fill="none" />
                      {/* Gotas coloridas */}
                      <circle cx="8" cy="8" r="2.5" fill={isActive(item) ? 'hsl(var(--primary))' : '#F44336'} />
                      <circle cx="16" cy="8" r="2.5" fill={isActive(item) ? 'hsl(var(--primary))' : '#2196F3'} />
                      <circle cx="8" cy="14" r="2.5" fill={isActive(item) ? 'hsl(var(--primary))' : '#FFEB3B'} />
                      <circle cx="16" cy="14" r="2.5" fill={isActive(item) ? 'hsl(var(--primary))' : '#4CAF50'} />
                    </svg>
                  ) : (
